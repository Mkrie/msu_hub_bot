...
def _children(node: _Node) -> Iterator[_Node]:
    if isinstance(node, list):
        ...
        if node.credit:
            yield "\n"
            yield node.credit
    elif isinstance(node, (RichBlockParagraph, RichBlockSectionHeading, RichBlockPreformatted, RichBlockFooter, RichBlockThinking)):
        yield node.text
    elif isinstance(node, RichBlockMathematicalExpression):
        yield node.expression
    elif isinstance(
        node,
        (RichBlockAnimation, RichBlockAudio, RichBlockDocument, RichBlockMap, RichBlockPhoto, RichBlockVideo, RichBlockVoiceNote),
    ):
        if node.caption:
            yield node.caption
