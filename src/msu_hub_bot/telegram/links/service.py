...
class LinkService:
    ...
    async def handle_video(self, message: Message, url: URL) -> None:
        try:
            result, timed_out = await self.executor.run(text_with_preview, str(url), timeout=60)
        except ExecutorBusy:
            return
    ...
    async def handle_native(self, message: Message, url: URL, site: LinkSite) -> None:
        ...
        providers = {"youtube": Provider.YOUTUBE, "instagram": Provider.INSTAGRAM, "tiktok": Provider.TIKTOK}
        try:
            with self.telemetry.operation(Boundary.PROVIDER, "links.extract", provider=providers[site]) as operation:
                result, timed_out = await self.executor.run(fetchers[site], str(url), timeout=85)
                if timed_out or not isinstance(result, LinkPost) or not result.assets:
                    operation.set_outcome(Outcome.TIMEOUT if timed_out else Outcome.UNAVAILABLE)
                    return
        except ExecutorBusy, ExternalServiceError, TimeoutError, ValueError, OSError:
            return
        try:
