...
class JDoodle(LoggerMixin):
    ...
    async def _request(self, endpoint: str, json: dict = None) -> bytes:
        json = json or {}
    ...
    async def credit_spent(self) -> JDoodleCreditResponse:
        ...
        return JDoodleCreditResponse.parse_raw(result)

    async def request(self, source_code: str, stdin: str = '', lang: str = 'python3', version: int = None) -> JDoodleResponse:
        if version is None:
            version = LANGUAGES[lang][1][-1][1]
        json = dict(script=source_code, stdin=stdin, language=lang, versionIndex=str(version))
        result = await self._request('execute', json=json)
        return JDoodleResponse.parse_raw(result)

    async def request_and_parse(self, source_code: str, stdin: str = '', lang: str = 'python3', version: int = None) -> str:
