...
async def tts(message: Message, meta: MetaInfo, lang: str = 'ru'):
    ...
    return await target.reply_audio(mp3, performer=f'TTS {lang.title()}', title=text[:20])


async def process_tts(message: Message, meta: MetaInfo):
    args = meta.arguments
    lang = args[0] if args else 'ru'
    return await tts(message, meta, lang)
