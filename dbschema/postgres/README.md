...
Migration 009 separates membership status clocks and provenance from activity, preserves existing evidence without fabricating status dates, and adds a receipt-independent membership ingestion RPC plus a bounded, state-filtered roster reader. The health capability `memberships: 1` gates new writers; older archive writers remain accepted. [Membership observations](../../docs/memberships.md) defines presence, ordering, pagination and coverage limits. Neither migration nor reader claims a complete Telegram roster or changes retention.
...
| `chat_users` | Candidate user/chat relationship or explicit membership evidence. Independent seen/status clocks, event/source ordering, status/permission snapshots and observed bot administrator-loss time. | Leaving updates status, retaining the row; no automatic deletion. The reader exposes `present`/`absent`/`unknown` as of evidence and incomplete coverage. |
