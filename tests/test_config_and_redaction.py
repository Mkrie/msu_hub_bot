...
from msu_hub_bot.redaction import RedactingFormatter, RedactingStream, before_send, redact
...
def test_json_collections_and_deployment_roundtrip(monkeypatch):
    ...
    payload = {"HUB_ORFOGRAMMKA_PASSWORD": secret, "HUB_FOUNDER_IDS": "[101, 202]", "HUB_JDOODLE_TOKENS": '[["client", "secret"]]'}
    for key in payload:
    ...
    config = Settings()
    assert config.orfogrammka_password == secret
...
def test_redacts_logs_tracebacks_streams_and_sentry(monkeypatch):
    secret = 'canary-value-$"/with-newline\nsecond-canary-line'
    monkeypatch.setattr(settings, "orfogrammka_password", secret)
    ...
    event = before_send({"extra": {"password": secret}, "exception": {"frames": [{"vars": {"password": secret}}]}}, {})
    assert secret not in json.dumps(event)
    assert "vars" not in event["exception"]["frames"][0]
